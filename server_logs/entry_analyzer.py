    ...
    if webserver_choice == str(1):
        ...
        if file_apache_choice == str(1):
            ...
            with open('access.log', 'r') as f:
                ...
                if match_lines_logs:
                    ...
                    if match_lines_logs.group(2):
                        ...
                        if sqli_patterns:
                            ...
                            if response == 200 and int(size) == 0:
                                print(
                                    Colors.red(f"[!] The response for the attack is 200 - OK with 0 bytes response..."))
                                print(Colors.red(f"[!] Please further check for the IOC "))
                            elif response == 200 and 0 < int(size) < 1200:
                                print(Colors.red(f"""[!] The response for the attack is 200 - OK 
                                                 The response size with {int(size)} bytes.... 
                ...
                """get working with the times frame for the most repeated IP"""
                print(Colors.yellow(
                    f"[*] Checking for suspicious short period requests that belong to {get_most} IP...."))
                if extracted_all:
                    count = 0
                    _3_first_matches = []
                    _3_last_matches = []
                    ...
                    for g in get_most:
                        ...
                        for get in get_back_content:
                            """^(\S+) - - \[(.*?)\] "(\S+ \S+ \S+)" (\d+) (\d+) "(.*?)" "(.*?)"""
                            # SAMPLE TO LOOK AT
                            match_all_specifically_day1 = re.match(
                                rf'^({re.escape(ip_add)}) - - \[({re.escape(str(match_most_busy_day[0]))})', get)
                            match_all_specifically_day2 = re.match(
                                rf'^({re.escape(ip_add)}) - - \[({re.escape(str(match_most_busy_day[1]))})', get)
                            match_all_specifically_day3 = re.match(
                                rf'^({re.escape(ip_add)}) - - \[({re.escape(str(match_most_busy_day[2]))})', get)
                            match_all_specifically_day4 = re.match(
                                rf'^({re.escape(ip_add)}) - - \[({re.escape(str(match_most_busy_day[3]))})', get)
                            match_all_specifically_day5 = re.match(
                                rf'^({re.escape(ip_add)}) - - \[({re.escape(str(match_most_busy_day[4]))})', get)
                            """FIVE TIMES"""
                            if match_all_specifically_day1:
                                print(Colors.cyan(f"""[+] {g[count]} Appears to be in the 1st most busy day from 
                                                                        {change_month} - Which Busy Month from the Logs...."""))
